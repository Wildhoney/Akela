...
<i>Distributed Server-Sent Events with tag-based fan-out over Redis pub/sub.</i>

[![Checks](https://github.com/Wildhoney/Akela/actions/workflows/checks.yml/badge.svg)](https://github.com/Wildhoney/Akela/actions/workflows/checks.yml)
[![Container](https://github.com/Wildhoney/Akela/actions/workflows/docker.yml/badge.svg)](https://github.com/Wildhoney/Akela/actions/workflows/docker.yml)
...
Released builds are published to the GitHub Container Registry, so you needn't build from source. Pull the image directly, or base your own image on it and bake in a config &mdash; the entrypoint is already the `akela` binary:

```dockerfile
FROM ghcr.io/wildhoney/akela:latest
COPY akela.yaml /etc/akela/akela.yaml
```

`ghcr.io/wildhoney/akela:latest` tracks `main`; every push is also tagged by commit SHA, and version tags such as `v1.2.3` publish matching `1.2.3` and `1.2` images.
